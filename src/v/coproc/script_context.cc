...
ss::future<either<script_context::write_response, model::term_id>>
script_context::write_checked(
  storage::log log,
  model::term_id last_term,
  model::record_batch_reader reader) {
    using rt_val = either<write_response, model::term_id>;
    ...
    return std::move(reader)
      .for_each_ref(
        ...
        model::no_timeout)
      .then([](auto rs) mutable {
          bool crc_success = std::get<0>(rs);
          if (!crc_success) {
              return ss::make_ready_future<rt_val>(write_response::crc_failure);
          }
          std::optional<model::term_id> newest_term = std::get<1>(rs);
          if (!newest_term) {
              return ss::make_ready_future<rt_val>(
                write_response::term_too_old);
          }
          ss::future written = std::move(std::get<2>(rs));
          return std::move(written).then(
  ...
  const model::materialized_ntp& m_ntp, model::record_batch_reader reader) {
    ...
    return found->second.first.with([this,
        ...
        return get_log(_resources.api, m_ntp.input_ntp())
          ...
          .then([this, m_ntp](either<write_response, model::term_id> written) {
